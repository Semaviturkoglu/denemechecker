...
import httpx  # requests yerine bunu kullanacağız
...
async def uret_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        key, sure_str = context.args; sure = int(sure_str)
        conn = sqlite3.connect(DB_NAME); cursor = conn.cursor()
    ...
    except (ValueError, IndexError): await update.message.reply_text("❌ Kullanım: `/uret YENIKEY 24`")
...
async def me_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Bu fonksiyon /me komutu içindir, butonlar için değil. Butonlar main_menu_callback kullanır.
    user_id = update.effective_user.id
...
async def check_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.callback_query:
        await update.callback_query.answer(); user_id = update.callback_query.from_user.id; chat_id = update.effective_chat.id
    else:
    ...
    if not await check_membership(user_id, context):
        await context.bot.send_message(chat_id=chat_id, text="❗️ Check için TÜM kanallara katılmalısınız. /start atın."); return ConversationHandler.END
    check_and_reset_credits(user_id); user_data = get_user(user_id); credits = user_data[2]; key_active = user_data[4] is not None
...
async def check_card_api(card: str, api_type: str) -> str:
    ...
    except httpx.TimeoutException: return f"API Hatası: Sunucu {timeout_duration} saniye içinde cevap vermedi."
    except httpx.RequestError as e: return f"API Bağlantı Hatası: {e}"
...
async def single_check_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏳ Kontrol ediliyor..."); api_response = await check_card_api(card_info, api_type)
...
async def mass_check_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, card in enumerate(cards):
        ...
        api_response = await check_card_api(card, api_type)
...
def main() -> None:
    ...
    handlers = [
        ...
        CommandHandler("uret", uret_command), CommandHandler("ban", ban_command),
        CommandHandler("profil", profil_command), CommandHandler("bakim", bakim_command),
        CommandHandler("aktifet", aktifet_command), CommandHandler("key", key_command),
